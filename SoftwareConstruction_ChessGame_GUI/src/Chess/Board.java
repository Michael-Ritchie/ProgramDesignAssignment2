...
public class Board {
    ...
    **/
    ...
    /**
     * Create a new file output stream, insert the current turn, for each square
     * on the grid, add the square position, the piece color, the piece letter,
     * and whether the piece has been moved. Close the new save file.
     * @param saveFile is a String parsed as the name to give the save file.
    **/
    public void saveBoard(String saveFile) {
        try {
            FileOutputStream fOut = new FileOutputStream(saveFile);
            PrintWriter pw = new PrintWriter(fOut);
            pw.println((whiteTurn) ? "T" : "F");
            for (int i = 0; i < 8; i++) {
                for (int j = 0; j < 8; j++) {
                    ChessPiece piece = board[i][j].getPiece();
                    if (piece != null) {
                        pw.println(Integer.toString(j) + Integer.toString(i) + ((piece.isWhite) ? "W" : "B") + piece.letter + ((piece.moved) ? "T" : "F"));
                    } else {
                        pw.println(Integer.toString(j) + Integer.toString(i));
                    }
                }
            }
            pw.close();
    ...
    **/
    public static Square moveToSquare(Square current, int rowMove, int colMove) {
    ...
    **/
    public boolean inCheck(ChessPiece king) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            possibleMoves = pieces[i].findMoves();
